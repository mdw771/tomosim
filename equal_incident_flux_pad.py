...
if __name__ == '__main__':
    ...
    prj_tomosaic.add_simuators(os.path.join('data', 'shepp_sino_trans.tiff'), inst, center=2048+2048, pixel_size=3.2,
    ...
    prj_local = Project()
    prj_local.add_simuators(os.path.join('data', 'shepp_sino_trans.tiff'), inst, center=2048+2048, pixel_size=3.2,
                               downsample=ds_local)

    if True:
        prj_tomosaic.process_all_tomosaic()
        prj_local.process_all_local(mask_ratio=0.85, offset_intensity=True, ref_fname='data/ref_recon.tiff', save_mask=True)
